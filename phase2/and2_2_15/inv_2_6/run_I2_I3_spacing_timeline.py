#!/usr/bin/env python3
"""Timeline: 2-I / 3-I spacing sweep in first 14 cycles (a31o / inv_2_6 arm).

Ones confined to cycles 0..13. First pulse at cyc 1; spacing Δ varies.

  2-I: I@{1, 1+Δ}     Δ=1..12  (1+Δ ≤ 13)
  3-I: I@{1, 1+Δ, 1+2Δ}  Δ=1..6  (1+2Δ ≤ 13)
...
N_CYC = 121
TAG = "and215_a31o_I2I3_space"
I_REF = 1
WIN_LAST = 13  # first 14 cycles: 0..13
...
def build_modes() -> list[tuple[str, str, list[int]]]:
    """Return (name, bitstring, ones_list)."""
...
def main() -> None:
    ...
    for ax, (name, _) in zip(axes, modes):
        ...
        ax.set_title(
            ...
            f"· inv.Y n={len(inv_y)} · a31o.X n={len(ones(rs, 'a31o_X'))}",
            fontsize=8,
            loc="left",
        )
    axes[-1].set_xlabel("cycle (yellow = first 14 · red ticks = I=1)")
    axes[0].legend(
    ...
    fig.suptitle(
        "inv_2_6 / a31o_2_11 · 2-I & 3-I spacing in first 14 cycles (I_REF=1)",
        fontsize=11,
    )
    fig.tight_layout()
    png = savefig_locked(fig, OUT / "I2_I3_spacing.png")

    # Summary table: sticky trip / inv.Y collapse
    md = [
        "# `inv_2_6` / `a31o_2_11` — 2-I / 3-I spacing (first 14 cycles)",
        "",
        f"Ones only in cycles `0..{WIN_LAST}`. Anchor `I_REF={I_REF}`.",
        "",
        "```text",
        "2-I : I@{1, 1+Δ}        Δ = 1..12",
        ...
        "",
        "Sibling (o211a pins): "
        "[`../o211a_2_8/timelines/I2_I3_spacing.md`](../o211a_2_8/timelines/I2_I3_spacing.md)",
        "",
        "## Sticky / fail observe",
        "",
    ...
    for name, _ in modes:
        ...
        md.append(
            ...
            f"{q[0] if q else '—'} | {len(iy)} | {len(ones(rs, 'a31o_X'))} | "
            f"{len(ones(rs, 'o211a_X'))} |"
        )
    md += [
        "",
        "## Per-mode lanes",
