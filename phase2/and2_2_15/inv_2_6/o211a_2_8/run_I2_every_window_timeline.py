#!/usr/bin/env python3
"""Exactly 2 I=1 in every period-11 window — o211a depth.

Same I patterns as inv_2_6/run_I2_every_window_timeline.py.
...
N_CYC = 121
TAG = "and215_o211a_I2_every_win"
PERIOD = 11
DELTAS = range(1, PERIOD)
...
def main() -> None:
    ...
    for ax, (name, _) in zip(axes, modes):
        ...
        ox = ones(rs, "o211a_X")
        ax.set_title(
            f"{name} · #I={len(ones_by[name])} · o211a.X n={len(ox)} "
            f"first={ox[0] if ox else '—'} · a22o.X n={len(ones(rs, 'a22o_X'))}",
            fontsize=8,
            loc="left",
        )
    ...
    fig.suptitle(
        "o211a_2_8 · exactly 2 I=1 per period-11 window · Δ=1..10 at {0, Δ}",
        fontsize=11,
    )
    fig.tight_layout()
    ...
    md = [
        ...
        "",
        f"Same I patterns as [`../../timelines/I2_every_window.md`](../../timelines/I2_every_window.md).",
        "",
        "```text",
    ...
    for name, _ in modes:
        ...
        md.append("")
        for lab, _n, title, _c in NOI_LANES + PIN_LANES:
    ...
    for name, _ in modes:
        rs = by_mode[name]
        ox = ones(rs, "o211a_X")
        print(f"  {name}: #I={len(ones_by[name])} o211a.X n={len(ox)}")
