#!/usr/bin/env python3
"""Timeline: exactly 2 I=1 in every period-11 window (Δ = 1..10).

Windows: [0..10], [11..21], …, [110..120]  (11 windows; N_CYC=121).

For each Δ:
  in every window w, I=1 at cycles  11*w  and  11*w + Δ

Usage (from rework_coded/):
  python3 phase2/and2_2_15/inv_2_6/run_I2_every_window_timeline.py
"""
from __future__ import annotations

import csv
import os
import sys
from pathlib import Path

os.environ.setdefault("MPLBACKEND", "Agg")
import matplotlib.pyplot as plt
from matplotlib.patches import Patch

HERE = Path(__file__).resolve().parent
_p = HERE
while not ((_p / "lib").is_dir() and (_p / "netlist").is_dir()):
    if _p.parent == _p:
        raise SystemExit(f"rework_coded root not found above {HERE}")
    _p = _p.parent
ROOT = _p
if str(ROOT / "lib") not in sys.path:
    sys.path.insert(0, str(ROOT / "lib"))

from probe_timeline import ones, pat_bits, run_probe  # noqa: E402
from rework_paths import savefig_locked  # noqa: E402

OUT = HERE / "timelines"
BUILD = ROOT / "phase2" / "build"
N_CYC = 121
TAG = "and215_I2_every_win"
PERIOD = 11
DELTAS = range(1, PERIOD)  # 1..10

LANES = [
    ("S", "sky130_fd_sc_hd__inv_2_8__A", "mux.S  inv_2_8__A", "#1f4e79"),
    ("A0", "sky130_fd_sc_hd__mux2_1_7__A0", "mux.A0  ¬(A∧I)", "#548235"),
    ("A1", "sky130_fd_sc_hd__or2_2_7__X", "mux.A1  A∨I", "#c45911"),
    ("X", "sky130_fd_sc_hd__mux2_1_7__X", "mux.X → a31o.A3", "#c00000"),
    ("a31o_X", "sky130_fd_sc_hd__a31o_2_11__X", "a31o.X", "#7030a0"),
    ("sticky_Q", "sky130_fd_sc_hd__inv_2_6__A", "sticky Q", "#833c0c"),
    ("inv_Y", "sky130_fd_sc_hd__inv_2_6__Y", "inv_2_6.Y → and2.A", "#833c0c"),
]


def ones_every_window(delta: int, off0: int = 0) -> list[int]:
    """Exactly 2 ones per period window: offsets off0 and off0+delta."""
    assert 0 <= off0 < PERIOD
    assert 1 <= delta < PERIOD
    assert off0 + delta < PERIOD
    pos: list[int] = []
    n_win = N_CYC // PERIOD
    for w in range(n_win):
        base = w * PERIOD
        pos.append(base + off0)
        pos.append(base + off0 + delta)
    return pos


def build_modes() -> list[tuple[str, str, list[int]]]:
    out: list[tuple[str, str, list[int]]] = [("all0", pat_bits(N_CYC), [])]
    for d in DELTAS:
        pos = ones_every_window(d, off0=0)
        name = f"I2win_d{d:02d}_off0"  # every win: @{0, d} relative
        out.append((name, pat_bits(N_CYC, pos), pos))
    return out


def fmt(xs: list[int], lim: int = 24) -> str:
    if len(xs) <= lim:
        return str(xs)
    return str(xs[:lim])[:-1] + f", …] (n={len(xs)})"


def main() -> None:
    mode_defs = build_modes()
    modes = [(n, b) for n, b, _ in mode_defs]
    ones_by = {n: pos for n, _b, pos in mode_defs}

    probes = [("I", "I"), ("enable", "enable")] + [(a, b) for a, b, *_ in LANES]
    rows = run_probe(
        root=ROOT,
        build=BUILD,
        out_dir=OUT,
        tag=TAG,
        probes=probes,
        modes=modes,
        n_cyc=N_CYC,
    )
    by_mode = {name: [r for r in rows if r["mode_name"] == name] for name, _ in modes}

    out_csv = OUT / "I2_every_window.csv"
    fields = ["mode", "cyc"] + [a for a, *_ in LANES]
    with out_csv.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fields)
        w.writeheader()
        for name, _ in modes:
            for r in by_mode[name]:
                w.writerow({k: r[k] if k != "mode" else name for k in fields})

    n_m = len(modes)
    fig, axes = plt.subplots(n_m, 1, figsize=(14, 1.55 * n_m), dpi=120, sharex=True)
    if n_m == 1:
        axes = [axes]
    for ax, (name, _) in zip(axes, modes):
        rs = by_mode[name]
        for yi, (lab, _net, title, col) in enumerate(reversed(LANES)):
            for c in ones(rs, lab):
                ax.barh(yi, 1.0, left=c - 0.5, height=0.72, color=col, edgecolor="none")
        for p in ones_by[name]:
            ax.axvline(p, color="#c00000", lw=0.55, alpha=0.35, zorder=0)
        for w in range(N_CYC // PERIOD + 1):
            ax.axvline(w * PERIOD - 0.5, color="#bbb", lw=0.6, ls=":", zorder=0)
        ax.set_yticks(range(len(LANES)))
        ax.set_yticklabels(
            [t for _a, _n, t, _c in reversed(LANES)], fontsize=6.2, family="monospace"
        )
        ax.set_xlim(-0.5, N_CYC - 0.5)
        ax.set_ylabel(name, fontsize=7, fontweight="bold")
        ax.grid(axis="x", color="#eee", lw=0.5)
        q = ones(rs, "sticky_Q")
        iy = ones(rs, "inv_Y")
        ax.set_title(
            f"{name} · #I={len(ones_by[name])} · sticky n={len(q)} "
            f"firstQ={q[0] if q else '—'} · inv.Y n={len(iy)} · mux.X n={len(ones(rs, 'X'))}",
            fontsize=8,
            loc="left",
        )
    axes[-1].set_xlabel("cycle (dotted = period-11 boundaries · red = I=1)")
    axes[0].legend(
        handles=[Patch(facecolor=c, label=lab) for lab, _n, _t, c in LANES],
        loc="upper right",
        ncol=4,
        fontsize=6.5,
        frameon=False,
    )
    fig.suptitle(
        "Exactly 2 I=1 per period-11 window · Δ=1..10 at offsets {0, Δ} · mux / sticky",
        fontsize=11,
    )
    fig.tight_layout()
    png = savefig_locked(fig, OUT / "I2_every_window.png")

    md = [
        "# Exactly 2 I=1 in every period-11 window",
        "",
        f"Period `{PERIOD}` · `{N_CYC // PERIOD}` windows covering `0..{N_CYC - 1}`.",
        "",
        "```text",
        "for each window w = 0..10:",
        "  I=1 at  11*w + 0   and   11*w + Δ     Δ = 1..10",
        "→ exactly 2 ones × 11 windows = 22 ones total",
        "```",
        "",
        f"Figure: [`{png.name}`]({png.name})",
        "",
        f"CSV: [`{out_csv.name}`]({out_csv.name})",
        "",
        "Sibling (o211a depth): "
        "[`../o211a_2_8/timelines/I2_every_window.md`](../o211a_2_8/timelines/I2_every_window.md)",
        "",
        "## Observe summary",
        "",
        "| mode | Δ | #I | sticky n | first Q | inv.Y n | mux.X n | a31o.X n |",
        "|------|--:|---:|---------:|--------:|--------:|--------:|---------:|",
    ]
    for name, _ in modes:
        rs = by_mode[name]
        pos = ones_by[name]
        d = (pos[1] - pos[0]) if len(pos) >= 2 else 0
        q = ones(rs, "sticky_Q")
        md.append(
            f"| `{name}` | {d} | {len(pos)} | {len(q)} | {q[0] if q else '—'} | "
            f"{len(ones(rs, 'inv_Y'))} | {len(ones(rs, 'X'))} | {len(ones(rs, 'a31o_X'))} |"
        )
    md += ["", "## Per-mode lanes", ""]
    for name, _ in modes:
        rs = by_mode[name]
        md.append(f"### `{name}`  I=`{fmt(ones_by[name])}`")
        md.append("")
        for lab, _n, title, _c in LANES:
            md.append(f"- `{lab}` ({title}) high @ `{fmt(ones(rs, lab))}`")
        md.append("")
    md += [
        "Parent: [`../README.md`](../README.md)",
        "",
        "```bash",
        "python3 phase2/and2_2_15/inv_2_6/run_I2_every_window_timeline.py",
        "```",
        "",
    ]
    md_path = OUT / "I2_every_window.md"
    md_path.write_text("\n".join(md), encoding="utf-8")
    print(f"wrote {md_path}")
    print(f"wrote {png}  ({n_m} modes)")
    for name, _ in modes:
        rs = by_mode[name]
        q = ones(rs, "sticky_Q")
        print(
            f"  {name}: #I={len(ones_by[name])} sticky n={len(q)} "
            f"inv.Y n={len(ones(rs, 'inv_Y'))}"
        )


if __name__ == "__main__":
    main()
